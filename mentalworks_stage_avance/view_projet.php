...
<html>
    ...
    <body>
        ...
        $q = $db->query('SELECT COUNT(*) FROM project');
        $items = $q->fetchAll();
        ...
        $totalItems = $db->query('SELECT count(id) AS total FROM project')->fetch(PDO::FETCH_ASSOC)['total'];
        ...
        $items = $db->query('SELECT * FROM project LIMIT '. $offset . ',' . $limit);
        ...
        <section id="view_page">
            <div class="container">
                ...
                echo "<h3 id='client'>$count projets <a href='projets/nouv_projet.php'><span class='glyphicon glyphicon-plus-sign' id='glyph'></span></a></h3>"; 
                ...
                <table class="table table-bordered titreTableau">
                    ...
                    <?php foreach ($items as $item): ?>
                        <tr class="contenuTableau">
                            ...
                            <td><?= $item['host_id']; ?></td>
                            <td><?= $item['managed_server']; ?></td>
                            <td><a href="projets/nouv_projet.php">Détails</a></td>
                        </tr>
                    <?php endforeach; ?>
                </table>
                ...
                <nav aria-label="Array pagination">
                    <ul class="pagination">
                        ...
                        <?php if($page != 1){ ?>
                            <li class="page-item">
                                <a class="page-link" href="view_projet.php?page=<?= $page - 1; ?>" aria-label="Previous" style="background-color: #f7f8f8; color:#000;">
                        ...
                        <?php for($i = 1;$i <= $totalPages;$i++) {?>
                            <li class="page-item <?= ($i == $page) ? 'active' : ''; ?>"><a class="page-link" style="background-color: #f7f8f8; color:#000;" href="view_projet.php?page=<?= $i; ?>"><?= $i; ?></a></li>
                        <?php } ?>
                        
                        <!-- next -->
                        <?php if($page < $totalPages){ ?>
                            <li class="page-item">
                                <a class="page-link"href="view_projet.php?page=<?= $page + 1; ?>" aria-label="Next" style="background-color: #f7f8f8; color:#000;">
