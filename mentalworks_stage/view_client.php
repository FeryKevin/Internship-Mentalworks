...
<html>
    ...
    <body>
        ...
        $totalItems = $db->query('SELECT count(id) AS total FROM customer')->fetch(PDO::FETCH_ASSOC)['total'];
        ...
        $items = $db->query('SELECT * FROM customer LIMIT '. $offset . ',' . $limit);
        ...
        <section id="view_page">
            <div class="container">
                ...
                echo "<h3 id='client'>$count clients <a href='clients/nouv_client.php'><span class='glyphicon glyphicon-plus-sign' id='glyph'></span></a></h3>"; 
...
<!--
                     input recherche    
                    <tr>
                        <td class="test"><input type='search' class='site-search'></td>
                        <td class="test"><input type='search' class='site-search'></td>
                        <td class="test"><input type='search' class='site-search'></td>
...
-->
                    ...
                    <?php foreach ($items as $item): ?>
                        <tr class="contenuTableau">
                            <td><?= $item['name']; ?></td>
                            <td><?= $item['code']; ?></td>
                            <td><a href="clients/update_client.php?id=<?php echo $item['id']?>" role="button">Modifier</a></td>
                        </tr>
                    <?php endforeach; ?>
                </table>
                ...
                <nav aria-label="Array pagination">
                    <ul class="pagination">
                        ...
                        <?php if($page != 1){ ?>
                            <li class="page-item">
                                <a class="page-link" href="view_client.php?page=<?= $page - 1; ?>" aria-label="Previous" style="background-color: #f7f8f8; color:#000;">
                        ...
                        <?php for($i = 1;$i <= $totalPages;$i++) {?>
                            <li class="page-item <?= ($i == $page) ? 'active' : ''; ?>"><a class="page-link" style="background-color: #f7f8f8; color:#000;" href="view_client.php?page=<?= $i; ?>"><?= $i; ?></a></li>
                        <?php } ?>
                        
                        <!-- next -->
                        <?php if($page < $totalPages){ ?>
                            <li class="page-item">
                                <a class="page-link"href="view_client.php?page=<?= $page + 1; ?>" aria-label="Next" style="background-color: #f7f8f8; color:#000;">
