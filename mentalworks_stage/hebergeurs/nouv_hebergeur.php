...
$db = connect();
$query = $db->query('SELECT MAX(id) AS maxid FROM host');
...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    if(empty($nom))
    {
        $nomErreur = "Veuillez saisir le nom de l'hébergeur.";
        $isSuccess = false;
    }
    ...
    if($isSuccess) 
    {
        ...
        if($results[0] == 0)
        {
            $statement = $db->prepare("INSERT INTO host (code, name) values(?, ?)");
        ...
        {
            $nomExiste = "Erreur : le nom de l'hébergeur est déjà utilisé.";
            $isSuccess = false;
        }
            ...
            <div class="container">
                <div class="row">
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12">
                        <p class="titre">Nouveau hébergeur</p>
                    </div>
